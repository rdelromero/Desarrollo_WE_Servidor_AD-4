...
drop user uventas_2024;
create user uventas_2024 identified by 'uventas';
grant all PRIVILEGES on ventasbbdd_2024.* to uventas_2024;

SELECT DISTINCT c.* FROM comerciales c INNER JOIN pedidos p ON c.id_comercial = p.id_comercial;

SELECT DISTINCT c.* FROM comerciales c INNER JOIN pedidos p ON c.id_comercial = p.id_comercial AND p.id_cliente=8